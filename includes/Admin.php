...
class Admin {
    ...
    public static function ajax_get_analytics() {
        ...
        error_log('[Admin.php] 📊 Site URL: ' . get_site_url());
        ...
        if (!$site_id) {
            error_log('[Admin.php] ❌ No site_id found - site not registered');
            wp_send_json_error(['message' => 'Site not registered. Please complete setup first.']);
            return;
        }
        
        error_log('[Admin.php] 📊 Site ID: ' . $site_id);
        
        // Call the correct endpoint for site-specific analytics
        $result = $api->get_site_analytics($site_id, $timeframe);
        
        error_log('[Admin.php] 📊 API result: ' . json_encode([
        ...
        if (!$result['success']) {
            error_log('[Admin.php] ❌ API call failed: ' . ($result['error'] ?? 'Unknown error'));
            wp_send_json_error(['message' => $result['error'] ?? 'Failed to fetch analytics']);
            return;
        }
        ...
        // Map backend metrics to frontend expected structure
        $metrics = $result['metrics'] ?? [];
        
        error_log('[Admin.php] 📊 Raw metrics from API: ' . json_encode($metrics));
        
        // Map the data correctly for overview.js
        $mapped_data = [
            'total_crawls' => $metrics['total_agent_crawls'] ?? 0,
            'total_paid' => $metrics['paid_agent_crawls'] ?? 0,
            'total_revenue' => $metrics['total_revenue'] ?? 0,
            'protected_pages' => $metrics['protected_pages'] ?? 0
        ];
        
        error_log('[Admin.php] 📊 Mapped data for frontend: ' . json_encode($mapped_data));
